...
def render(df, ARTICLE_PRESETS, analysis_type=""):
    # If no valid type passed from routing, default to the first one
    if analysis_type not in _ANALYSIS_TYPES:
        analysis_type = _ANALYSIS_TYPES[0]

    # CRPD Article Coverage
    if analysis_type == "CRPD Article Coverage":
        st.markdown(
            f"### {_ms('menu_book')} CRPD Article Coverage Analysis",
    ...
    # Keywords & Topics
        ...
        st.markdown(
            f"### {_ms('chat_bubble')} Keyword & Topic Analysis",
            unsafe_allow_html=True,
        )
        ...
        with col2:
            ...
            fig.update_layout(height=500, margin=dict(l=20, r=20, t=40, b=20), yaxis_title=None)
            fig.update_yaxes(automargin=True)
            st.plotly_chart(fig, width="stretch")
    ...
    # Comparative Analysis
        ...
        st.markdown(
            f"### {_ms('compare_arrows')} State Reports vs. Committee Analysis",
            unsafe_allow_html=True,
        )
        ...
        if len(sr) and len(co):
            ...
            with col1:
                ...
                if not sr_art.empty:
                    sr_top = (
                        sr_art.groupby("article")["count"].sum().reset_index().nlargest(10, "count")
                    )
                    fig = px.bar(
                        sr_top,
                        x="count",
            ...
            with col2:
                ...
                if not co_art.empty:
                    co_top = (
                        co_art.groupby("article")["count"].sum().reset_index().nlargest(10, "count")
                    )
                    fig = px.bar(
                        co_top,
                        x="count",
    ...
    # Model Shift Analysis
        ...
        st.markdown(
            f"### {_ms('balance')} Medical Model vs. Rights-Based Model Analysis",
            unsafe_allow_html=True,
        )
        ...
        if len(mt):
            # Global trend
            by_year = (
                mt.groupby("year")[["medical", "rights"]].sum().reset_index().sort_values("year")
            )
            fig = px.area(
                by_year,
                x="year",
            ...
            if "region" in mt.columns:
                ...
                fig = px.bar(
                    by_region.sort_values("rights_pct"),
                    x="rights_pct",
                    y="region",
                    orientation="h",
    ...
    # Article Deep-Dive Analysis
        ...
        st.markdown(
            f"### {_ms('manage_search')} Article-Specific Deep-Dive Analysis",
            unsafe_allow_html=True,
        )
        st.markdown("""
        ...
        st.markdown(
            f"### {_ms('bar_chart')} Analysis for **{selected_article}**",
            unsafe_allow_html=True,
        )
        ...
        st.caption(
            f"**Tracking keywords:** {', '.join(article_keywords[:8])}{'...' if len(article_keywords) > 8 else ''}"
        )

        # Calculate mentions across filtered data
        with st.spinner("Analyzing article mentions..."):
            article_mentions = df["clean_text"].apply(lambda t: count_phrases(t, article_keywords))
        ...
        if len(df_with_article) == 0:
            st.warning(
                f"❌ No mentions of **{selected_article}** found in the currently filtered data. Try adjusting your filters in the sidebar."
            )
        else:
            # Key Metrics
            st.markdown(
                f"#### {_ms('trending_up', '1em')} Key Metrics",
                unsafe_allow_html=True,
            )
            ...
            # Geographic Analysis
            st.markdown(
                f"#### {_ms('public', '1em')} Geographic Distribution",
                unsafe_allow_html=True,
            )
            col1, col2 = st.columns(2)
            ...
            with col1:
                by_region = (
                    df_with_article.groupby("region")["article_mentions"].sum().reset_index()
                )
                ...
                if not by_region.empty:
                    ...
                    st.plotly_chart(fig, width="stretch")
                    st.caption(
                        f"📊 {selected_article.split('—')[0].strip()} is most discussed in {by_region.iloc[0]['region']}"
                    )

            with col2:
                by_country = (
                    df_with_article.groupby("country")["article_mentions"].sum().reset_index()
                )
                ...
                if not by_country.empty:
                    ...
                    st.plotly_chart(fig, width="stretch")
                    st.caption(
                        f"🌍 {by_country.iloc[0]['country']} mentions this article most frequently"
                    )

            # Document Type Analysis
            st.markdown("---")
            st.markdown(
                f"#### {_ms('description', '1em')} Document Type Analysis",
                unsafe_allow_html=True,
            )
            ...
            with col1:
                by_doctype = (
                    df_with_article.groupby("doc_type")["article_mentions"].sum().reset_index()
                )
            ...
            if not by_doctype.empty:
                ...
                st.caption(
                    f"📋 Most mentioned in **{top_doctype['doc_type']}** ({top_doctype['article_mentions']} mentions)"
                )
            ...
            if "year" in df_with_article.columns and len(df_with_article) > 0:
                st.markdown("---")
                st.markdown(
                    f"#### {_ms('calendar_month', '1em')} Temporal Trends",
                    unsafe_allow_html=True,
                )

                by_year = (
                    df_with_article.groupby("year")["article_mentions"]
                    .sum()
                    .reset_index()
                    .sort_values("year")
                )
                if not by_year.empty:
                    ...
                    if len(by_year) >= 2:
                        ...
                        if first_year_mentions > 0:
                            pct_change = (
                                (last_year_mentions - first_year_mentions)
                                / first_year_mentions
                                * 100
                            )
                            trend_emoji = (
                                "📈" if pct_change > 0 else "📉" if pct_change < 0 else "➡️"
                            )
                            st.caption(
                                f"{trend_emoji} Mentions changed by {pct_change:+.1f}% from {int(by_year.iloc[0]['year'])} to {int(by_year.iloc[-1]['year'])}"
                            )

            # Top Documents
            st.markdown("---")
            st.markdown(
                f"#### {_ms('sort', '1em')} Documents with Most Mentions",
                unsafe_allow_html=True,
            )
            ...
            render_accessible_table(
                display_columns(top_docs),
                caption=f"Top 15 documents mentioning {selected_article.split('—')[0].strip()}",
            )
            ...
            if "Article 13" in selected_article:
                st.markdown("---")
                st.markdown(
                    f"#### {_ms('balance', '1em')} Article 13 Spotlight: Access to Justice",
                    unsafe_allow_html=True,
                )
                st.caption(
                    "Additional analysis drawn from justice-related language in the filtered documents."
                )
                ...
                with col2:
                    ...
                    if len(df_with_article):
                        justice_total = (
                            df_with_article["clean_text"]
                            .apply(lambda t: count_phrases(t, justice_keywords))
                            .sum()
                        )
                        concern_total = (
                            df_with_article["clean_text"]
                            .apply(lambda t: count_phrases(t, concern_keywords))
                            .sum()
                        )
                        ...
                        fig = px.bar(
                            ...
                            color="Category",
                            color_discrete_map={
                                "Justice Access Terms": CATEGORICAL_PALETTE[0],
                                "Concern/Barrier Terms": CATEGORICAL_PALETTE[1],
                            },
                        )
                        fig.update_layout(height=450, showlegend=False)
                        st.plotly_chart(fig, width="stretch")
                        st.caption(
                            "Higher concern language signals areas flagged by the Committee."
                        )

    # Article Co-occurrence Analysis
    elif analysis_type == "Article Co-occurrence":
        st.markdown(
            f"### {_ms('share')} Article Co-occurrence Analysis",
            unsafe_allow_html=True,
        )
        st.markdown("""
        ...
        if not len(df):
            ...
            with st.spinner(
                "Computing co-occurrence matrix… This may take a moment on first load."
            ):
                try:
                    ...
                    if art_freq_co.empty:
                        st.warning(
                            "No article matches found for current filters. Try adjusting your filters in the sidebar."
                        )
                    else:
                        top_arts = (
                            art_freq_co.groupby("article")["count"]
                            .sum()
                        ...
                        if len(top_arts) < 2:
                            st.warning(
                                f"Only {len(top_arts)} article(s) found. Need at least 2 articles for co-occurrence analysis. Try adjusting your filters."
                            )
                        else:
                            # Build a binary presence matrix: documents × top articles
                            presence = {
                                art: df["clean_text"].apply(
                                    lambda t, art=art: int(
                                        count_phrases(t, ARTICLE_PRESETS[art]) > 0
                                    )
                                )
                                for art in top_arts
                            }
                            ...
                            for i in range(len(arts)):
                                for j in range(i + 1, len(arts)):
                                    ...
                                    if val > 0:
                                        pairs.append(
                                            {"Pair": f"{arts[i]}  +  {arts[j]}", "Documents": val}
                                        )
                            ...
                            if pairs:
                                ...
                                fig_pairs = px.bar(
                                    ...
                                    labels={
                                        "Documents": "Documents Mentioning Both",
                                        "Pair": "Article Pair",
                                    },
                                    color="Documents",
                                    color_continuous_scale="Blues",
                    ...
                    st.error(f"An error occurred while computing co-occurrence: {e!s}")
                    st.info(
                        "This may be due to insufficient data, your current filter settings, or a timeout. Try adjusting your filters or selecting a smaller data subset."
                    )
