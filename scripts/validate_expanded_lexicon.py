...
def compare_lexicons(df):
    ...
    print(
        f"  Change: +{len(MODEL_DICT['Medical Model']) - len(OLD_MODEL_DICT['Medical Model'])} terms"
    )
    print()
    ...
    print(
        f"  Change: +{len(MODEL_DICT['Rights-Based Model']) - len(OLD_MODEL_DICT['Rights-Based Model'])} terms"
    )
...
def analyze_sample_documents(df, n_samples=5):
    ...
    for _idx, row in sample_df.iterrows():
        ...
        print(
            f"  Old: Medical={old_counts['Medical Model']}, Rights={old_counts['Rights-Based Model']}, Rights%={old_rights_pct:.1f}%"
        )
        print(
            f"  New: Medical={new_counts['Medical Model']}, Rights={new_counts['Rights-Based Model']}, Rights%={new_rights_pct:.1f}%"
        )
        print(
            f"  Change: Medical +{new_counts['Medical Model'] - old_counts['Medical Model']}, Rights +{new_counts['Rights-Based Model'] - old_counts['Rights-Based Model']}, Rights% {new_rights_pct - old_rights_pct:+.1f}pp"
        )
...
def main():
    ...
    print(
        f"✅ Total term occurrences increased: {results['old_medical'] + results['old_rights']:,} → {results['new_medical'] + results['new_rights']:,}"
    )
    print(
        f"✅ Rights-based percentage: {results['old_rights_pct']:.2f}% → {results['new_rights_pct']:.2f}%"
    )
    print("\n" + "=" * 80 + "\n")
