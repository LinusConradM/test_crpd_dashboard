...
def calculate_document_coverage(df):
    ...
    if total_docs == 0:
        ...
        print(
            f"Documents with Rights-Based Model terms only: {len(rights_docs)} ({len(rights_docs) / total_docs * 100:.1f}%)"
        )
        print(
            f"Documents with BOTH model terms: {len(both_docs)} ({len(both_docs) / total_docs * 100:.1f}%)"
        )
        print(
            f"Documents with NEITHER model terms: {len(neither_docs)} ({len(neither_docs) / total_docs * 100:.1f}%)"
        )
    ...
    for model_name, results in lexicon_results.items():
        ...
        if model_low_freq:
            for r in model_low_freq:
                print(
                    f"  - {r['term']}: {r['doc_count']} documents, {r['total_count']} total occurrences"
                )
                low_freq_terms.append((model_name, r["term"], r["doc_count"], r["total_count"]))
        else:
            print("  (No terms below threshold)")
...
def main():
    ...
    # 2. Calculate document coverage
    calculate_document_coverage(df)

    # 3. Extract candidate terms
    extract_candidate_terms(df, top_n=100)

    # 4. Identify low-frequency terms
    identify_low_frequency_terms(lexicon_results, threshold=10)
